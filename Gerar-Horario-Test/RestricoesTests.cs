...
{
    public static void ProfessorNaoPodeTrabalharEmTresTurnosDiferentesTest(GerarHorarioContext contexto, IEnumerable<HorarioGerado> horarioGerado)
    {
        foreach (var str in horarioGerado)
        {
            foreach (var professor in contexto.Options.Professores)
            {
                ...
                {
                var propostasManha = from aula in str.Aulas
                                     ...
                                     select aula;
                var propostasTarde = from aula in str.Aulas
                                     where aula.ProfessorId == professor.Id
                                     && aula.DiaDaSemanaIso == diaSemanaIso
                                     &&
                                     (
                ...
                var propostasNoite = from aula in str.Aulas
                                     where aula.ProfessorId == professor.Id
                                     && aula.DiaDaSemanaIso == diaSemanaIso
                                     &&
                                     (
